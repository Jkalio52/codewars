...
The HashInclude Speech Processing team has a project named Virtual Assistant. 
...
The number of vowels in each of these substrings is 0, 1, 1, 2, 2, 1, 1, 2, 2, 0, 1, 1, 1, 1, 0; if you sum up these numbers, you get 16 - the expected output.
...
function vowelRecognition(input) {
  ...
  for (let i = 0; i < n; i++) {
    if (vowels.has(input[i])) {
      // The number of substrings that include the character at index i is
      // the number of starting positions (i + 1) multiplied by the number
      // of ending positions (n - i).
